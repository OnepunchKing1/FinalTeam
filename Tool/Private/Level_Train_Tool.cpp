#include "pch.h"
#include "..\Public\Level_Train_Tool.h"
...
CLevel_Train_Tool::CLevel_Train_Tool(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CLevel(pDevice, pContext)
{

}

HRESULT CLevel_Train_Tool::Initialize()
{
    ...
    {
        MSG_BOX("Failed to Ready_Lights : CLevel_Train_Tool");
        return E_FAIL;
    }

    if (FAILED(Ready_Layer_BackGround(TEXT("Layer_BackGround"))))
    {
        MSG_BOX("Failed to Ready_Layer_BackGround : CLevel_Train_Tool");
        return E_FAIL;
    }
...
void CLevel_Train_Tool::Tick(_double dTimeDelta)
{
    ...
    /*if (GetKeyState(VK_RETURN) & 0x8000)
    {
        ...
        hr = pGameInstance->Open_Level(LEVEL_LOADING, CLevel_Loading_Tool::Create(m_pDevice, m_pContext, LEVEL_FINALBOSS));
...
HRESULT CLevel_Train_Tool::Render()
{
...
HRESULT CLevel_Train_Tool::Ready_Lights()
{
    CGameInstance* pGameInstance = CGameInstance::GetInstance();
    Safe_AddRef(pGameInstance);
...
HRESULT CLevel_Train_Tool::Ready_Layer_BackGround(const _tchar* pLayerTag)
{
    CGameInstance* pGameInstance = CGameInstance::GetInstance();
    Safe_AddRef(pGameInstance);
...
HRESULT CLevel_Train_Tool::Ready_Layer_Camera(const _tchar* pLayerTag)
{
    CGameInstance* pGameInstance = CGameInstance::GetInstance();
    Safe_AddRef(pGameInstance);
...
HRESULT CLevel_Train_Tool::Ready_Layer_Player(const _tchar* pLayerTag)
{
    CGameInstance* pGameInstance = CGameInstance::GetInstance();
    Safe_AddRef(pGameInstance);
...
HRESULT CLevel_Train_Tool::Ready_Layer_MapObject(const _tchar* pLayerTag)
{
...
HRESULT CLevel_Train_Tool::Load_MapObject_Info(const _tchar* pPath, const _tchar* pLayerTag)
{
    CGameInstance* pGameInstance = CGameInstance::GetInstance();
    Safe_AddRef(pGameInstance);
...
CLevel_Train_Tool* CLevel_Train_Tool::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CLevel_Train_Tool* pInstance = new CLevel_Train_Tool(pDevice, pContext);
    
    if (FAILED(pInstance->Initialize()))
    {
        MSG_BOX("Failed to Created : CLevel_Train_Tool");
